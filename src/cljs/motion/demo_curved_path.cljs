  ...
  (let [over (atom false)]
    (fn []
      [:g {:on-mouse-over (fn [ev] (reset! over true) nil) :on-mouse-out (fn [ev] (reset! over false) nil)}
       (component-svg-hexagon (assoc style :fill "url(#hatch)") x y 40)
       (if @over
         [:g
...
(defn component-svg-twolines [style]
  [:g {:transform "translate(0,100)"}
   [:path (merge style {:d (js/roundPathCorners (str "M -200 45 L 48 45 L 98 5 L 200 5") 5 false) :stroke-width "2px"})]
   [:path (merge style {:d (js/roundPathCorners (str "M -200 50 L 50 50 L 100 10 L 200 10") 5 false) :stroke-width "2px"})]])
